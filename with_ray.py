...
for f in categorical_features:
    lbl = LabelEncoder()
    # Include 'Unknown' in classes to handle unseen labels
    lbl.fit(list(train_df[f].astype(str).values) + ['Unknown'])
...
param_grids = {
    ...
    'SVR': {
        ...
        'epsilon': tune.loguniform(1e-3, 1e-1),
        'kernel': tune.choice(['rbf', 'linear'])
    }
}

def tune_hyperparameters_ray(model, param_distributions, train_X, train_y):
    # Initialize the search algorithm
    search_alg = BayesOptSearch(metric="r2", mode="max")
    
    # Initialize TuneSearchCV with desired configuration
...
def train_and_evaluate(model, train_X, train_y):
    ...
    if hasattr(model, 'coef_') or hasattr(model, 'feature_importances_') or hasattr(model, 'n_features_in_'):
...
for threshold in variance_thresholds:
    ...
    for model_name, model in models.items():
        ...
        if model_name in param_grids:
            logging.info(f"Tuning hyperparameters for {model_name} using Ray Tune...")
            best_model, best_params = tune_hyperparameters_ray(model, param_grids[model_name], train_X_pca, train_y)
        ...
        logging.info(
            f"{model_name}: R²={r2:.4f}, Adjusted R²={adj_r2:.4f}, MAE={mae:.4f}, "
